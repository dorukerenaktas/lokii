...
def mock_loader(mocker):
    def patch(exists: bool = False, code: Any = "") -> None:
        ...
        mocker.patch("importlib.machinery.SourceFileLoader.get_code", lambda *args: code),

    return patch


def test_load_raise_error_if_file_not_found(mock_loader):
    mock_loader(False)
...
def test_load_should_exec_loaded_module(mock_loader):
    code = 'conf = {"test_env": {}}'
    mock_loader(True, compile(code, "/test/path/any.py", "exec"))
    loader = ModuleFileLoader("/test/path/any.py")
    loader.load()
